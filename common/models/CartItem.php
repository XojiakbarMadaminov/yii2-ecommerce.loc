...
class CartItem extends \yii\db\ActiveRecord
{
        ...
        if (\Yii::$app->user->isGuest) {
            ...
            $cartItems = CartItem::findBySql("
                            ...
                            From cart_items c
                                     ...
                                     where c.created_by = :userId", [':userId' => $currUserId])->asArray()->all();
        }
        return $cartItems;
    }
    ...
    public static function getTotalPriceForUser($currUserId)
    {
        if (\isGuest()) {
            ...
            $sum = CartItem::findBySql("
            SELECT SUM(quantity * price) FROM cart_items left join products p on cart_items.product_id = p.id WHERE cart_items.created_by = :userId", [':userId' => $currUserId])
                ->scalar();
        }
        return $sum;
    }
    ...
    public static function getTotalPriceForItemForUser($productId, $currUserId)
    {
        if (\isGuest()) {
            ...
            $sum = CartItem::findBySql("
            SELECT SUM(quantity * price) FROM cart_items left join products p on cart_items.product_id = p.id WHERE cart_items.product_id = :id AND cart_items.created_by = :userId", [':id' => $productId, ':userId' => $currUserId])
                ->scalar();
        }
        return $sum;
    }
